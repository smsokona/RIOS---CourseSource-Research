...
# REALIZE THERE MAY NOT NECCESSARILY BE AN ERROR BUT YUHAO IS USING THE WORD COUNT FROM THE EXCEL SHEET WHICH MAY INCLUDE STOP WORDS..HOWEVER THE DEI WORD COUNT IS STILL PRETTY HIGH...

# COMMENTING OUT YUHAO'S CODE BC I DON'T THINK IT'S CORRECT... SEE CODE BELOW
...
#     if( rios_data_tokenized$dei_relatedit[k] == "TRUE" && rios_data_tokenized$Title[k] == rios_data$Title[i] ){
...
# 
# WHEN I PRINT `rios_data$`DEI Word count``, FOR ARRTICLE NO. 5 AND 6. THEIR DEI WORD COUNT IS 26 AND 41 RESPECTIVELY, HOWEVER, WHEN I PRINT THE CODE BELOW, I GET DISCREPENCIES.
# # article 5: 4 vs 26, article 6: 9 vs 41
# rios_data_tokenized %>%
#   filter(dei_relatedit == "TRUE") %>%
#   count(article_num)
# 
# rios_data_tokenized %>% 
#   filter(article_num == 5 & dei_relatedit == "TRUE")
# 
# rios_data_tokenized %>% 
#   filter(article_num == 6 & dei_relatedit == "TRUE")

# USE ORIGINAL BOX PLOT CODE, NOT CODE FROM LAST NIGHT, BUT EDIT CODE ABOVE AND SHOW YUHAO DISCREPENCY 
...
# SIMILAR CODE AS ABOVE, BUT COUNT'S DUPLICATES (CODE ABOVE COUNT DISTINCT DEI WORDS AND TOTAL WORDS)
totaldeiarticle1 <- rios_data_tokenized %>%
  filter(dei_relatedit == "TRUE") %>%
  count(article_num)

totalwordsarticle1 <- rios_data_tokenized %>% 
  count(article_num)

totaldeiarticle1 %>%
  full_join(totalwordsarticle1, by = c("article_num")) %>%
