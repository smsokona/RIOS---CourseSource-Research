...
knitr::opts_chunk$set(warning = FALSE, # doesn't print warnings 
...
text(base_barplot, par("usr")[3], srt = 45, labels = c("2014-18", "2019-22") , adj = c(1.1,1.1), xpd = TRUE, cex=.79)
...
Below, I create a plot of the average *JEDI* word count, comparing the word count for articles which include authors that attended CourseSource workshops and those that didn't. I also conduct a one-sample t-test. Although the average JEDI word count is much lower here, we can see that those who've attended workshops have a relatively higher average word count, like we see above.

```{r avg. dei wrd ct bar plot by workshop attendance, dev='png', dpi = 300}

# store data for bar plot
dei_by_workshop_data <- rios_data %>% 
  group_by(`Attended Workshop?`) %>% 
  # calculate avg. wrd ct, length, and standard deviation for each group
  summarise(Avg_word_count = mean(`DEI Word count`, na.rm = TRUE), 
            n = n(),
            sd = sd(`DEI Word count`, na.rm = TRUE)) %>% 
  # calculate the standard error and  the standard error multiplied by the value of the t-distribution for 0.5
  mutate(se = sd/sqrt(n), 
         ic = se * qt((1-0.05)/2 + 0.5, n-1))

# store the base barplot in a variable 
base_barplot2 <- barplot(dei_by_workshop_data$Avg_word_count ~ dei_by_workshop_data$`Attended Workshop?`, axis.lty = 1, ylim = c(0,40), col = c("#a6cee3", "#1f78b4"), xlab = "Attendance Type", ylab = "Average Word Count", names.arg = dei_by_workshop_data$`Attended Workshop?`)
...
# conduct a one sample t-test
jeditestbyattendance <- t.test(formula = `DEI Word count` ~ `Attended Workshop?`, data = rios_data)
...
Looking at proportions gives us an idea of how many DEI words are being used and if there is a change over time. Below, I calculate the number of DEI related words for each article and year. Then, I divided those numbers by the total amount of words for each article and year to get a proportion. Based on what Yuhao calculated in section [Paper-wise statistics], we already have a column which gives the total amount of DEI related words for each article, and the ratio. I use this column to calculate the total amount of DEI related words for each *year*. Since I want to calculate the total amount of words (including stopwords), for 2-word and 3-word phrases I repeated some of my analyses done in the *Data Cleaning* RMD. I couldn't use the `rios_data_tokenized2`, and `rios_data_tokenized3` dataframes, since they're cleaned and don't contain stopwords. The total amount of words and DEI related words I calculate are not *distinct* and include duplicates.

```{r Proportion of DEI Words dfs}

# calculate the total amount of DEI related words for each YEAR
totaldeiyear <- rios_data %>% 
  # group by year
  group_by(Year) %>% 
  # select the `DEI Word Count` column
  select(13) %>% 
  # add the DEI Word Count column for each year
  summarise(totaldei_by_year = sum(`DEI Word count`, na.rm = FALSE))
...
totalwordsyear <- rios_data %>%
  # group by year
  group_by(Year) %>%
  # select the `Word Count` column
  select(9) %>% 
  # add the Word Count column for each year
  summarise(total_by_year = sum(`Word Count of Inclusive Teaching?`))
...
total2deiyear <- rios_data_tokenized2 %>%
  # filter for words labeled as DEI related
  filter(dei_related == "TRUE") %>%
  # group by year
  group_by(Year) %>%
  # count the number of times each article appears
  count(article_num) %>%
  # add the number of times the article appears for each year
  summarise(totaldei_by_year = sum(n)) 

# calculate the total amount of DEI related 2-word phrases for each ARTICLE
total2deiarticle <- rios_data_tokenized2 %>%
  # filter for words labeled as DEI related
  filter(dei_related == "TRUE") %>%
  # group by article no. & year
  group_by(article_num, Year) %>%
  # count the number of times each article appears
  count(article_num) 

# store unnecessary punctuation or digits in a vector for removal
strings <- c("[:punct:]", "[:digit:]","\\(","\\)")

# calculate the total amount of 2-word phrases in each YEAR
total2wordsyear <- rios_data %>%
...
total2wordsarticle <-  rios_data %>%
  ...
  # count the total in each article
  tally()
...
total3wordsyear <- rios_data %>%
  ...
  # remove ~1,994 rows of punctuation and digits
...
Here's a box plot of the ratio of DEI related words, 2-word phrases and 3-word phrases by article over time with a table which goes more in depth of the statistics shown on the boxplot for each year. For some reason, the mean in 2014 or 2015 is the highest across 1 word, 2-word and 3 phrases. However, the ratio shifts and decreases as we move from 1 words to 3 words. The average ratio is slightly higher from the years 2019 to 2022 for 1 word (0.135 vs. 0.134) and 2 word phrases (0.0745 vs. 0.0723).
...
df1 <- rios_data %>%
  select(4, 9, 11, 13, 14) %>%
  # turn the ratio into a decimal
  mutate(`DEI_Words_%` = `DEI_Words_%`/100) %>%
  group_by(Year)  %>% 
  rename("Article Number" = "article_num",
         "Total DEI By Article" = "DEI Word count",
         "Total By Article" = "Word Count of Inclusive Teaching?",
         "DEI Ratio 1w" = "DEI_Words_%")
...
ratioboxplot <- function(){
  ...
  abline(v = 0.134, col = "#b2df8a", lty = "solid", lwd = 3)
    
  # color and impose horizontal line of 2019 - 2022 group average ratio
  abline(v = 0.135, col = "#1f78b4",lty = "solid", lwd = 3)
      
  # impose legend which explain horizontal lines
  legend("bottomleft", inset=.02, title="Average Ratio", c("for 2014 - 2018","for 2019 - 2022"), fill=c("#b2df8a", "#1f78b4"), horiz=FALSE, cex=0.8)
...
For article numbers 69 and 95, there aren't any DEI related 2-word phrases, which is why we're seeing 2 NA's in the table below.

```{r ratio boxplot and skim table for 2word}

df2 <- total2deiarticle %>%
  full_join(total2wordsarticle, by = c("article_num")) %>%
  # calculate the ratio of DEI "relatedness"
  mutate(ratio = n.x/n.y) %>%
  group_by(Year)  %>% 
  rename("Article Number" = "article_num",
         "Total DEI By Article" = "n.x",
         "Total By Article" = "n.y",
         "DEI Ratio 2w" = "ratio")
...
ratio2boxplot <- function(){
  ...
  # color and impose horizontal line of 2014 - 2018 group average ratio
  abline(v = 0.0723, col = "#b2df8a", lty = "solid", lwd = 3)
    
  # color and impose horizontal line of 2019 - 2022 group average ratio
  abline(v = 0.0745, col = "#1f78b4",lty = "solid", lwd = 3)
...
For article numbers 5, 59, 69, 95, 128, 140, 142, 219, and 247, there aren't any DEI related 3-word phrases, which is why we're seeing 9 NA's in the table below.

```{r ratio boxplot and skim table for 3word}

df3 <- total3deiarticle %>%
  full_join(total3wordsarticle, by = c("article_num")) %>%
  # calculate the ratio of DEI "relatedness"
  mutate(ratio = n.x/n.y) %>%
  group_by(Year)  %>%
  rename("Article Number" = "article_num",
...
ratio3boxplot <- function(){
  ...
  # color and impose horizontal line of 2014 - 2018 group average ratio
  abline(v = 0.0512, col = "#b2df8a", lty = "solid", lwd = 3)
    
  # color and impose horizontal line of 2019 - 2022 group average ratio
  abline(v = 0.0480, col = "#1f78b4",lty = "solid", lwd = 3)
