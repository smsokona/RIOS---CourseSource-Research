...
# SHOW CARRIE AND SUGGEST TAB GROUPS...

# require(gridExtra)
 grid.arrange(base_bargraph_over_time, base_lineplot, ncol = 2)


# arrange two plots into one column
plot_grid(
  base_bargraph_over_time, base_lineplot, ncol = 1
)
...
```{r Proportion of DEI Words 2, eval=FALSE}
...
```{r Proportions of DEI Phrases, eval=FALSE}
...
```{r Proportions of DEI Phrases 2, eval=FALSE}
...
```{r Proportions of DEI Phrases 3w 2, eval=FALSE}
#proportions for each article, also fluctuating
total3deiarticle %>%
  full_join(totalwordsarticle, by = c("article_num")) %>%
  mutate(ratio = totaldeia/totala) %>%
