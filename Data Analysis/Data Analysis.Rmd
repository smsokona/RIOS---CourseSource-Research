...
```{r wrd ct boxplot and barchart, dev = "png"}
...
```{r barplot and line graph of wrd ct over time, dev='png', warning=FALSE, message = FALSE}
...
Looking at proportions gives us an idea of how many DEI words are being used and if there is a change over time. Since we're grouping the data by year and article, the total DEI words for each year and article will be higher than 761, as some words are used in multiple years/articles.

I've calculated the number of DEI related words for each article and year. Then, I divided those numbers by the total amount of words for each article and year to get a proportion. Even though we have a word count column, I used the cleaned data frame `rios_data_tokenized`, so I'm actually dividing by the total amount of cleaned words.
...
Here's a box plot of the ratio of DEI related words, 2-word phrases and 3-word phrases by article over time. The ratio increases as we move from 1 words to 3 words, however, there are smaller instances of phrases, which may be why the ratio is increasing. For example, for article number 16, there's only one 3-word phrase, which is labeled as DEI related; therefore the ratio here is 100%. 
...
boxplot(df1$`DEI Ratio 1w` ~ df1$Year, df1, xlab = "Year", ylab = "Ratio", main = "The Ratio of DEI Related Words Over Time", horizontal = FALSE,  col = c("#b2df8a","#b2df8a", "#b2df8a", "#b2df8a", "#b2df8a", "#1f78b4", "#1f78b4", "#1f78b4", "#1f78b4"), ylim = c(0,1)) 
...
boxplot(df2$`DEI Ratio 2w` ~ df2$Year, df2, xlab = "Year", ylab = "Ratio", main = "The Ratio of DEI Related 2-Word Phrases Over Time", horizontal = FALSE,  col = c("#b2df8a","#b2df8a", "#b2df8a", "#b2df8a", "#b2df8a", "#1f78b4", "#1f78b4", "#1f78b4", "#1f78b4"), ylim = c(0,1)) 
...
boxplot(df3$`DEI Ratio 3w` ~ df3$Year, df3, xlab = "Year", ylab = "Ratio", main = "The Ratio of DEI Related 3-Word Phrases Over Time", horizontal = FALSE,  col = c("#b2df8a","#b2df8a", "#b2df8a", "#b2df8a", "#b2df8a", "#1f78b4", "#1f78b4", "#1f78b4", "#1f78b4"), ylim = c(0,1)) 
...
Below are line graphs of the ratio of DEI related words, 2-word phrases and 3-word phrases by year over time. The ratio increases as we move from 1 words to 3 words, however, there are smaller instances of phrases, which may be why the ratio is increasing. For example, for article number 16, there's only one 3-word phrase, which is labeled as DEI related; therefore the ratio here is 100%.  
...
totaldeiyear %>%
  ...
  # calculate the ratio of DEI "relatedness"
  mutate(ratio = totaldei_by_year/total_by_year	) %>%
  ggplot(aes(Year, ratio)) +
  geom_line() +
  ylim(0,0.38) +
  labs(y = "Ratio", title = "The Ratio of DEI Related Words By Year", subtitle = "(in the 'Inclusive Teaching' Section)")

```
 

I decided to group by article number as well for a better, more detailed look at the changes in the proportions. Because we're looking at the proportions by article the proportions are higher on this graph. Regardless, we can see in 2014 has the highest proportions. However, articles 224, 211 (both in Year 2021), and 105 (in Year 2019) have some of the highest proportions as well.
...
```{r Proportions of DEI Phrases, warning=FALSE, message=FALSE}
  
total2deiyear %>%
  full_join(total2wordsyear, by = c("Year")) %>%
  # calculate the ratio of DEI "relatedness"
  mutate(ratio = totaldei_by_year/total_by_year	) %>%
  ggplot(aes(Year, ratio)) +
  geom_line() +
  ylim(0,0.38) +
  labs(y = "Ratio", title = "The Ratio of DEI Related 2-Word phrases By Year", subtitle = "(in the 'Inclusive Teaching' Section)")
...
```{r Proportions of DEI Phrases 3w, eval=FALSE}
  
total3deiyear %>%
  full_join(total3wordsyear, by = c("Year")) %>%
  # calculate the ratio of DEI "relatedness"
  mutate(ratio = totaldei_by_year/total_by_year	) %>%
  ggplot(aes(Year, ratio)) +
  geom_line() +
  ylim(0,0.38) +
  labs(y = "Ratio", title = "The Ratio of DEI Related 3-Word phrases By Year", subtitle = "(in the 'Inclusive Teaching' Section)")
