...
My name is Sokona Mangane and I'm from Brooklyn, NY. I'm a senior at Bates College, majoring in Mathematics, and minoring in Digital and Computational Studies. 

ADD ROLE


DO NOT KNIT SO THAT YOU SEE CODE ON THE HTML
...
rios_data_tokenizedit <- rios_data_tokenizedit[,-c(10:13)]
...
top_10_distinct <- it_word_counts %>% 
  slice(1:10) %>% # return top 10 word counts for each year
  ungroup(Year) %>% # ungroup by year
  distinct(inclusive_teach_tokens) # return distinct words across the df
...
# original
it_word_counts %>%
  filter(inclusive_teach_tokens %in% top_10_distinct) %>% # using only 14.17% DEI related words in dataset
  mutate(log_n = log(n)) %>% 
